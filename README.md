...
| Area | Status |
|---|---|
| Email/template/block builder | Working. Text/heading/button elements have font controls in the Inspector's Appearance section: font-family, font size, a color picker (native swatch paired with a hex field), and strikethrough. The font-family list is mostly standard web-safe stacks (Arial, Helvetica, Georgia, Verdana, etc.) plus two Google Fonts (Roboto, Red Hat Text) loaded via a `<link>` that MJML auto-injects and Outlook desktop is deliberately excluded from loading — so those two render as the real font everywhere except Outlook desktop, where they cleanly fall back to Arial instead of failing. Building the font-family picker caught a real bug: `mj-text`/`mj-button` don't accept a generic `style` attribute at all, so these fields had never actually been applied in real rendered output before (MJML's soft validation swallowed the error) — confirmed by direct testing against `mjml2html`, now fixed. |
...
| Contacts | Working — CRUD, org-scoped. Contact Lists too: create/rename/delete a list, add/remove members, and select a whole list as recipients from the builder's Send modal. Custom fields (text/number/date/url) via the "Custom fields" button on the Contacts page — define org-wide fields, set per-contact values from the New/Edit Contact modal. Deleting a contact who's already received a campaign is blocked with a clear error (they're part of the send/unsubscribe audit trail) rather than failing with a raw DB error; list membership and custom field values are cleaned up automatically otherwise. |
...
| Domain verification | Working, in-app — the Domains page (`/domains`) calls Resend's Domains API directly, so you never have to leave the app to add a sending domain: add a domain, it returns the DNS records to add at your registrar, then "Check verification" re-checks status. Requires `RESEND_API_KEY`. This is the one delivery-provider integration in this pass that hasn't been exercised against the live Resend API from this environment — the request/response shapes match Resend's published Domains API, but verify by actually adding a real domain before relying on it. |
