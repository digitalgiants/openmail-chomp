...
| Area | Status |
|---|---|
| Email/template/block builder | Working. Text/heading/button elements have font controls in the Inspector's Appearance section: font-family (limited to standard web-safe stacks like Arial/Georgia/Verdana — no custom/Google Fonts, since Outlook desktop doesn't support `@font-face` and a font that silently fails for a chunk of recipients isn't a real feature), font size, a color picker (native swatch paired with a hex field), and strikethrough. Building the font-family picker caught a real bug: `mj-text`/`mj-button` don't accept a generic `style` attribute at all, so these fields had never actually been applied in real rendered output before (MJML's soft validation swallowed the error) — confirmed by direct testing against `mjml2html`, now fixed. |
